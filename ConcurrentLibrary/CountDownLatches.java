...
// * 
// * 		- Other tasks may call countDown() on the object to reduce the count, presumably when a task finishes its
// job
...
// * 
// * 		A typical use is to divide a problem into n independently solvable tasks and create a CountDownLatch with
// a value of n.
...
class CountDownLatchesWorker implements Runnable {

	private int id;
	private CountDownLatch latch;
	private Random random;
	
	public CountDownLatchesWorker(int id, CountDownLatch latch) {
		this.latch = latch;
		this.id = id;
...
public class CountDownLatches {
	...
	public static void main(String[] args) {
		...
		runCyclicCountDownLatch();
		runCyclicCountDownLatch();
	}

	private static void runCyclicCountDownLatch() {
		ExecutorService executorService = Executors.newSingleThreadExecutor();
		CountDownLatch latch = new CountDownLatch(5);

		IntStream.range(0, 5).mapToObj(i -> new CountDownLatchesWorker(i + 1, latch)).forEach(executorService::execute);
